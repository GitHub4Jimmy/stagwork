...
{
    public class FindTrainingLocationByFilterLinkProvider : BaseProvider, ILinkProvider
    {
        ...
        {
            var dnnLink = LinkHelper.GetDnnUrl(KeyIdentifiers.PAGE_TRAINING_LOCATIONS_KEY);

            if (!String.IsNullOrEmpty(dnnLink)
                && link.DatabaseIds.ContainsKey(KeyIdentifiers.LOCATIONS_FILTER_KEY))
            {
                var queryParameter =
                    $"{TrainingFundSearchHelper.FILTER_PRE_APPEND}{link.DatabaseIds[KeyIdentifiers.LOCATIONS_FILTER_KEY]}";

                int idCount = 1;
                var value = String.Empty;

                while (link.DatabaseIds.ContainsKey($"{KeyIdentifiers.LOCATIONS_SEARCH_KEY}{idCount}"))
                {
                    value += $"{link.DatabaseIds[KeyIdentifiers.LOCATIONS_SEARCH_KEY + idCount]}|";
                    idCount++;
                }

...
{
    public class RecommendedCoursesService
    {
        public async Task<MPRecommendationBoxViewModel> Get(int personId, string targetLanguage = "en")
        {
            var globalSettings = DummyContentSettingsHelper.GetSettings();

            if (globalSettings.UseDebugContent)
            {
                return DummyContentHelper.Load<MPRecommendationBoxViewModel>("RecommendedCourses");
            }
            ...
            try
            {
                ...
                MPRecommendationBoxViewModel model = null;

                HttpResponseMessage response = await client.GetAsync($"recommended-courses?personId={personId}");

                if (response.IsSuccessStatusCode)
                {
                    model = await response.Content.ReadAsAsync<MPRecommendationBoxViewModel>();
                }

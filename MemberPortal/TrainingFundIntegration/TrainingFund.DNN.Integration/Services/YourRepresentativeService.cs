...
{
    public class YourRepresentativeService
    {

        public async Task<MPSiteManagerBoxViewModel> Get(int personId, string targetLanguage = "en")
        {
            var globalSettings = DummyContentSettingsHelper.GetSettings();

            if (globalSettings.UseDebugContent)
            {
                return DummyContentHelper.Load<MPSiteManagerBoxViewModel>("YourRepresentative");
            ...
            try
            {
                ...
                MPSiteManagerBoxViewModel model = null;

                HttpResponseMessage response = await client.GetAsync($"your-representative?personId={personId}");

                if (response.IsSuccessStatusCode)
                {
                    model = await response.Content.ReadAsAsync<MPSiteManagerBoxViewModel>();
                }

...
{
    public class CoursesController : DnnApiController, IRequiresSessionState
    {
        ...
        public async Task<Response> SendResults()
        {
            ...
            var service = new TrainingFindCourseService();
            ...
            var result = await service.Get(personId, searchTerm, filters, lang);

            result.Results.Items.ForEach(c =>
            {
                c.Name.Link = LinkHelper.GetLinkWithJumpUrl(c.Name);
            });

            return await Mail.SendTrainingFundFindCourseResults(UserInfo.Email, result.Results);

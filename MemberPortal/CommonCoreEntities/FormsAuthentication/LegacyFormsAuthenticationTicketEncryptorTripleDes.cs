...
namespace StagwellTech.SEIU.CommonCoreEntities.FormsAuthentication
{
    public class LegacyFormsAuthenticationTicketEncryptorTripleDes
    {
        ...
        public LegacyFormsAuthenticationTicketEncryptorTripleDes(string decryptionKey, string validationKey, ShaVersion hashAlgorithm = DefaultHashAlgorithm, CompatibilityMode compatibilityMode = DefaultCompatibilityMode)
        {
        ...
        public LegacyFormsAuthenticationTicketEncryptorTripleDes(byte[] decryptionKey, byte[] validationKey, ShaVersion hashAlgorithm = DefaultHashAlgorithm, CompatibilityMode compatibilityMode = DefaultCompatibilityMode)
        {
        ...
        private void Initialize(byte[] decryptionKey, byte[] validationKey, ShaVersion hashAlgorithm, CompatibilityMode compatibilityMode)
        {
            ...
            _DecryptionKeyBlob = KeyDerivatorTripleDes.DeriveKey(decryptionKey, _CompatibilityMode);

            _hasher = HashProvider.Create(KeyDerivatorTripleDes.DeriveKey(validationKey, _CompatibilityMode), hashAlgorithm);
        }
        ...
        public FormsAuthenticationTicket DecryptCookie(string cookieString)
        {
            ...
            return FormsAuthenticationTicketHelperTripleDes.Deserialize(decryptedCookie, ticketLength);
        }

        private byte[] EncryptCookieData(byte[] cookieBlob, int length, HashProvider hasher = null)
        {

            using (var aesProvider = TripleDES.Create())
            {
                aesProvider.Key = _DecryptionKeyBlob;
                aesProvider.BlockSize = 64;
        ...
        public string Encrypt(FormsAuthenticationTicket ticket, bool randomiseUsingHash = false)
        {
            ...
            byte[] ticketBlob = FormsAuthenticationTicketHelperTripleDes.Serialize(ticket);
            if (ticketBlob == null)
            {
